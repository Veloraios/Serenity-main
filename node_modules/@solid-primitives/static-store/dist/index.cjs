...
function createStaticStore(init) {
  ...
  const getValue = (key) => {
    ...
    if (!signal) {
      if (!solidJs.getListener()) return copy[key];
  ...
  const setValue = (key, value) => {
    ...
    if (key in copy) copy[key] = utils.accessWith(value, copy[key]);
  };
  return [
    store,
    (a, b) => {
...
function createHydratableStaticStore(serverValue, update) {
  if (web.isServer) return createStaticStore(serverValue);
  ...
  for (const key in store)
    Object.defineProperty(store, key, {
      get() {
        ...
        if (!keyMemo) {
          if (!solidJs.getListener()) return fnMemo()[key];
